    ...
    for (const issue of issues) {
      if (issue.createdByUserId) {
        ...
        if (!options.has(id)) {
          options.set(id, {
            id,
            label: formatAssigneeUserLabel(issue.createdByUserId, currentUserId, companyUserLabelMap)
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId, companyUserLabelMap) ?? tx("User"))
            : (agentName(key) ?? key.slice(0, 8)),
      items: groups[key]!,
    }));
  }, [agentName, companyUserLabelMap, currentUserId, filtered, issueTitleMap, t, tx, viewState.groupBy, workspaceNameMap]);
                                  ...
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <Identity
                                          name={assigneeUserLabel ?? tx("User")}
