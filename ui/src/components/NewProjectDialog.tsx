...
import { PROJECT_COLORS } from "../../../packages/shared/src/constants.js";
...
export function NewProjectDialog() {
  ...
  const projectStatuses = [
    { value: "backlog", label: t("project.statusBacklog") },
    { value: "planned", label: t("project.statusPlanned") },
  ...
  const deriveWorkspaceNameFromPath = (value: string) => {
    ...
    return segments[segments.length - 1] ?? t("project.localFolder");
  };
  ...
  const deriveWorkspaceNameFromRepo = (value: string) => {
    try {
      ...
      return repo || t("project.githubRepo");
    } catch {
      return t("project.githubRepo");
    }
  };
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("project.localFolderMustBeAbsolute"));
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("project.repoMustBeValid"));
      return;
    }
      ...
      >
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("project.repoUrl")}</label>
              <span className="text-xs text-muted-foreground/50">{t("project.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("project.repoUrlHelp")}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("project.localFolder")}</label>
              <span className="text-xs text-muted-foreground/50">{t("project.optional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder={t("project.localFolderPlaceholder")}
              />
              <ChoosePathButton />
            </div>
          </div>
          ...
          {selectedGoals.map((goal) => (
            ...
            >
              ...
              <button
                ...
                aria-label={t("project.removeGoal", { goal: goal.title })}
                type="button"
              >
                <X className="h-3 w-3" />
              </button>
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("project.addGoal") : t("project.goal")}
              </button>
            </PopoverTrigger>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("project.failedToCreateProject")}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("issueProperties.creating") : t("project.createProject")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
