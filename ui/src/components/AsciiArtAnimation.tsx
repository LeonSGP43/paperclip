...
type FlowSprite = (typeof FLOW_SPRITES)[number];
...
function spriteSize(sprite: FlowSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = FLOW_SPRITES[(baseCol + baseRow) % FLOW_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = FLOW_SPRITES[Math.floor(Math.random() * FLOW_SPRITES.length)]!;
