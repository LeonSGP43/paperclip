...
function formatParticipantLabel(participant: ActivityParticipant, options: ActivityFormatOptions, t: Translate): string {
  ...
  const profile = participant.userId ? options.userProfileMap?.get(participant.userId) : null;
  return profile?.label ?? formatUserLabel(participant.userId, options.currentUserId, t);
}
