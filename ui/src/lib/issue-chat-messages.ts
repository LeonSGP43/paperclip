...
function createTimelineEventMessage(args: {
  ...
  const actorName = event.actorType === "agent"
    ...
    : event.actorType === "system"
      ? t("common.system")
      : (formatAssigneeUserLabel(event.actorId, currentUserId, userLabelMap) ?? t("common.board"));
  ...
  if (event.assigneeChange) {
    const from = event.assigneeChange.from.agentId
      ...
      : (formatAssigneeUserLabel(event.assigneeChange.from.userId, currentUserId, userLabelMap) ?? t("issueList.unassigned"));
    const to = event.assigneeChange.to.agentId
      ...
      : (formatAssigneeUserLabel(event.assigneeChange.to.userId, currentUserId, userLabelMap) ?? t("issueList.unassigned"));
